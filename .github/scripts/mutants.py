...
# shape - the skipped leg is credited at full scale - so the test observes the Router's guard.
 ...
 dict(nome="V4 promise: the up clamp counts the range from the tick's start again (one tick late)",
      f="src/BlazePhoenixCore.sol",
      old="        uint256 d = zeroForOne ? uint256(r) : uint256(sp_ - r - 1);",
      new="        uint256 d = zeroForOne ? uint256(r) : uint256(sp_ - r); // MUTANT",
      teste="test_Up_PriceInsideItsTick_PromiseNeverExceedsTheRangeOutput"),
 ...
 dict(nome="promise: the leg under-attests (every upper-bound check still holds)",
      f="src/BlazePhoenixSolver.sol",
      old="                        if (p != 0 && p < one) one = p;",
      new="                        if (p != 0 && p < one) one = p / 2; // MUTANT",
      teste="test_TheLegAttestsThePromise_TheHopKeepsTheCapacity"),
 dict(nome="promise: the hop capacity is overwritten by the promise (ranking would move)",
      f="src/BlazePhoenixSolver.sol",
      old="            sum += one;",
      new="            sum += one; hop.expectedOut = sum; // MUTANT",
      teste="test_TheLegAttestsThePromise_TheHopKeepsTheCapacity"),
 dict(nome="V4 promise: the up arm of outV3 ignores its limit (the bound was computed by outV3 itself)",
      f="src/BlazePhoenixCore.sol",
      old="            if (sqrtLimit != 0 && sqrtNew > sqrtLimit) sqrtNew = sqrtLimit;",
      new="            // MUTANT: the up arm is never truncated",
      teste="test_Up_PriceInsideItsTick_PromiseNeverExceedsTheRangeOutput"),
