...
M = [
 ...
 dict(nome="fee: so no hop 0 (a fuga que a ancora na entrada tinha)",
      f="src/BlazePhoenixRouter.sol",
      old="        uint256 feeH = BPC.mulDiv(baseH, PROTOCOL_FEE_BPS, BPC.BPS);",
      new="        uint256 feeH = h == 0 ? BPC.mulDiv(baseH, PROTOCOL_FEE_BPS, BPC.BPS) : 0; // MUTANTE",
      teste="test_JUIZ_PrefixoSemValorEscapaAFee"),
 ...
 dict(nome="quoter: a fee do preview compoe por hop",
      ...
      old="            afterFee -= BPC.mulDiv(afterFee, PROTOCOL_FEE_BPS, BPC.BPS);",
      new="            afterFee = route.totalOut - BPC.mulDiv(route.totalOut, PROTOCOL_FEE_BPS, BPC.BPS); // MUTANTE",
