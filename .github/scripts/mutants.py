...
M = [
 ...
 dict(nome="V4 promise: the edge-blind single-tick form again, instead of the walk",
      f="src/BlazePhoenixCore.sol",
      old="        return v4WalkOut(manager, poolId, amountIn, effV4Fee(keyFee, lpF, pF, zeroForOne), tickSpacing, zeroForOne);",
      new="        return outV3(amountIn, sp, lq, effV4Fee(keyFee, lpF, pF, zeroForOne), zeroForOne, 0); // MUTANT",
...
# shape - the skipped leg is credited at full scale - so the test observes the Router's guard.
 ...
 dict(nome="kind: aceitar a declaracao da calldata contra a forma medida",
      f="src/BlazePhoenixCore.sol",
      old="        if (declaredConc != isConc) return (false, kind, 0);",
 ...
 dict(nome="kind: nao derivar a familia concentrada da forma da pool",
      f="src/BlazePhoenixCore.sol",
      old="        if (isConc) k = dynShape ? KIND_ALGEBRA : KIND_V3;",
      new="        isConc; // MUTANTE",
 ...
 dict(nome="callback: o unlockCallback V4 aceita quem nao e o PoolManager",
      ...
      new="        mgr; // MUTANTE",
      # Re-watched 2026-09-23: the old test sent an empty payload and the mutant died in
      # abi.decode, not here. The guard only decides while the transient context is live.
      teste="test_V4ImpostorCallingUnlockCallbackMidSwapIsNeverPaid"),
 ...
 # ── V4 campaign (2026-09-23): guards with no test able to see them ─────────
 # A coverage map of the V4 surface found these guards unwatched, or watched by a
 # test that died for another reason. Each now has a test that sees it.
 dict(nome="Quoter: the preview stops asking the Router's hook question (a paused hook reads executable)",
      f="src/BlazePhoenixQuoter.sol",
      old="        pv.canExecute = pv.netOut > 0 && pv.netOut >= pv.effectiveMinOut && !_hookPausedIn(route);",
      new="        pv.canExecute = pv.netOut > 0 && pv.netOut >= pv.effectiveMinOut; // MUTANT",
      teste="test_PreviewRoute_PausedHookLeg_CannotExecute"),
 dict(nome="V4 settle: the sync before paying is dropped",
      f="src/BlazePhoenixRouter.sol",
      old="            IV4PoolManager(mgr).sync(tIn);",
      new="            // MUTANT: no sync",
      teste="test_HonestRoute_PoolA_Settles"),
 dict(nome="V4 settle: the settle is dropped (a delta left open at the end of the unlock)",
      f="src/BlazePhoenixRouter.sol",
      old="            IV4PoolManager(mgr).settle();",
      new="            // MUTANT: no settle",
      teste="test_HonestRoute_PoolA_Settles"),
 dict(nome="V4 delta sign: the received-delta arm is dropped",
      f="src/BlazePhoenixRouter.sol",
      old="        if (owedDelta > 0 || receivedDelta < 0) revert RouterE(8);",
      new="        if (owedDelta > 0) revert RouterE(8); // MUTANT",
      teste="test_G3_V4Settle_NegativeReceivedDelta_Reverts8"),
 dict(nome="V4 budget: a leg may be charged more than it committed (the sibling legs pay)",
      f="src/BlazePhoenixRouter.sol",
      old="        if (owe > amt) revert RouterE(8);",
      new="        // MUTANT: no per-leg budget",
      teste="test_V4LegCannotPullSiblingBudget"),
 dict(nome="receive: the open window accepts any sender",
      f="src/BlazePhoenixRouter.sol",
      old="        if (ok == 0 || msg.sender != address(uint160(ok))) revert RouterE(3);",
      new="        if (ok == 0) revert RouterE(3); // MUTANT",
      teste="test_Receive_RejectsStrangerInsideOpenWindow_AndMgrOutsideIt"),
 dict(nome="V4 execution: a delta-returning hook is no longer refused",
      f="src/BlazePhoenixRouter.sol",
      old="            if (BPC.hookAltersDeltas(leg.hooks)) revert RouterE(9);",
      new="            // MUTANT: delta hooks pass",
      teste="test_ADeltaHookIsRefusedBeforeTheManagerIsEverContacted"),
 dict(nome="Quoter: a delta-returning hook is priced again",
      f="src/BlazePhoenixQuoter.sol",
      old="        if (leg.hooks != address(0) && BPC.hookAltersDeltas(leg.hooks)) return 0;",
      new="        // MUTANT: delta hooks priced",
      teste="test_Exact_V4_DeltaHookUnquotable"),
 dict(nome="Quoter: a V4 leg with no counter token is dry-run anyway",
      f="src/BlazePhoenixQuoter.sol",
      old="        if (tokenOther == address(0)) return 0;",
      new="        // MUTANT: no counter-token guard",
      teste="test_Exact_V4_ZeroAuxTokenRefused"),
 dict(nome="V4 swap: the price limits are swapped (every swap aims the wrong way)",
      f="src/BlazePhoenixRouter.sol",
      old="            sqrtPriceLimitX96: zfo ? BPC.MIN_SQRT_PRICE_PLUS_ONE : BPC.MAX_SQRT_PRICE_MINUS_ONE",
      new="            sqrtPriceLimitX96: zfo ? BPC.MAX_SQRT_PRICE_MINUS_ONE : BPC.MIN_SQRT_PRICE_PLUS_ONE // MUTANT",
      teste="test_HonestRoute_PoolA_Settles"),
 dict(nome="V4 pool id: encodePacked instead of abi.encode (not the id the manager publishes)",
      f="src/BlazePhoenixCore.sol",
      old="        return keccak256(abi.encode(currency0, currency1, fee, tickSpacing, hooks));",
      new="        return keccak256(abi.encodePacked(currency0, currency1, fee, tickSpacing, hooks)); // MUTANT",
      teste="test_ThePublishedBasePoolIdIsReproducedByOurDerivation"),
 dict(nome="V4 extsload: liquidity read one slot off",
      f="src/BlazePhoenixCore.sol",
      old="            mstore(add(m, 4), add(base, 3))",
      new="            mstore(add(m, 4), add(base, 2)) // MUTANT",
      teste="test_V4SqrtAndLiq_DecodesAllFiveFields_NegativeTick"),
 ...
 dict(nome="kind: the pair-shaped family takes the declaration again (V2 vs Solidly)",
      f="src/BlazePhoenixCore.sol",
      old="        if (!isConc) k = isSolidlyShaped(pool) ? KIND_SOLIDLY : KIND_V2;",
      new="        isConc; // MUTANTE",
 ...
 dict(nome="registry: a pair-shaped row's fee comes from the V3 getter again",
      f="src/BlazePhoenixCore.sol",
      old="        fee = isConc ? (dynShape ? 0 : getV3Fee(pool)) : 0;",
      new="        fee = dynShape ? 0 : getV3Fee(pool); // MUTANTE",
 ...
 dict(nome="S2b: seedPool admits a hook that addV4 would refuse",
    ...
    dict(nome="REG-01: the registry fee comes from calldata again instead of the pool's shape", f='src/BlazePhoenixHub.sol', old='            (proven, kind, feeReg) = BPC.provenShape(pool, kind);', new='            (proven, kind, ) = BPC.provenShape(pool, kind); // MUTANT: the calldata fee stays', teste='test_FeeForjadaNoCalldataNaoEntraNoRegisto'),
 ...
 # ── META-AUDIT, ninth wave: tests that could not fail, and the mutants that now prove they can ──
 # Each entry below names a test the meta-audit found weak or decorative; the mutant is the
 # smallest edit that test used to survive. Killed = the test now watches the line.
 dict(nome="V4 budget: capped by the whole Router balance, not by the leg budget",
      f="src/BlazePhoenixRouter.sol",
      old="        if (owe > amt) revert RouterE(8);",
      new="        if (owe > BPC.balanceOf(tIn, address(this))) revert RouterE(8); // MUTANT",
      teste="test_V4LegCannotPullSiblingBudget"),
 dict(nome="empty book (L799c2 witness): the split clamp skips a book holding no tokenOut",
      f="src/BlazePhoenixSolver.sol",
      old="            if (BPC.kindHas(cands[i].kind, BPC.A_CONC_POOL)) {",
      new="            if (BPC.kindHas(cands[i].kind, BPC.A_CONC_POOL) && balsOut[i] > 0) { // MUTANT",
      teste="test_L799c2_AZeroBalanceConcBookCarriesNoLeg"),
 dict(nome="empty book (delivery witness): the split clamp skips a book holding no tokenOut",
      f="src/BlazePhoenixSolver.sol",
      old="            if (BPC.kindHas(cands[i].kind, BPC.A_CONC_POOL)) {",
      new="            if (BPC.kindHas(cands[i].kind, BPC.A_CONC_POOL) && balsOut[i] > 0) { // MUTANT",
      teste="test_AnEmptyBookSeatedBesideTheRouteNeverCostsTheUserAnything"),
 dict(nome="V4 identity (fuzz witness): the Quoter prices a leg whose pool is not its key",
      f="src/BlazePhoenixQuoter.sol",
      old="        if (leg.pool != address(uint160(uint256(BPC.computeV4PoolId(c0, c1, leg.fee, leg.tickSpacing, leg.hooks))))) return 0;",
      new="        // MUTANT",
      teste="testFuzz_Quoter_PricesIffFieldsAgree"),

 # ── NINTH WAVE, remainder: each fix and the smallest edit that reopens it ──
 dict(nome="dex-13: the quote channel promises the pair's figure, not the executor's ask",
      f="src/BlazePhoenixCore.sol",
      old="            out = solidlyAskOut(c.pool, amountIn, c.tokenIn, c.zeroForOne, c.stable, c.fee);",
      new="            out = solidlyGetAmountOut(c.pool, amountIn, c.tokenIn); // MUTANT",
      teste="test_ThePublishedNetOutOfASolidlyRouteIsAMinOutTheRouterHonours"),
 dict(nome="Brian Wahyu: the gate measures only the deepest survivor at full size",
      f="src/BlazePhoenixSolver.sol",
      old="                Hop memory alt = _singleLeg(tIn, tOut, amountIn, cands[i], allowCut);",
      new="                Hop memory alt = single; // MUTANT",
      teste="test_TheGateComparesTheSplitWithEverySurvivorAtFullSize"),
 dict(nome="dex-16: the operator's door writes a kind the shape contradicts",
      f="src/BlazePhoenixHub.sol",
      old="            if (!proven) revert HubE(4);",
      new="            proven; // MUTANT",
      teste="test_TheOperatorDoorRefusesAKindTheShapeContradicts"),
 dict(nome="dex-16: the operator's door keeps the declared fee on a concentrated row",
      f="src/BlazePhoenixHub.sol",
      old="            if (BPC.kindHas(k, BPC.A_CONC_POOL)) fee = f;",
      new="            f; // MUTANT",
      teste="test_TheOperatorDoorWritesTheFeeTheShapeReports"),
 dict(nome="dex-17: a seeded row is born at depth bucket 0 again",
      f="src/BlazePhoenixHub.sol",
      old="            BPC.depthBucket(BPC.registryDepth18(pool, kind, t0, t1, $.v4PoolManager, pid)));",
      new="            BPC.depthBucket(0)); // MUTANT",
      teste="test_ASeededDeepPoolIsNotCutFromTheFunnelByWarmDust"),
 dict(nome="dex-12: the newcomer is scored without the pair's bridge term",
      f="src/BlazePhoenixHub.sol",
      old="        if (bridged) newcomerPsi += (newcomerPsi * 2_500) / BPC.BPS;",
      new="        bridged; // MUTANT",
      teste="test_ANewcomerOnABridgedPairCarriesTheBridgeTermItsRivalsCarry"),
 # ── THE V4 WALK (dex-19): the pool's own book, walked; each guard and the edit that reopens it ──
 dict(nome="V4 walk: ranking goes back to the edge-blind single-tick form (dex-19 reopens)",
      f="src/BlazePhoenixCore.sol",
      old="            out = v4WalkOut(c.v4Manager, pid, amountIn, effV4Fee(c.fee, lpF, pF, c.zeroForOne), c.tickSpacing, c.zeroForOne);",
      new="            out = outV3(amountIn, sp, liq, effV4Fee(c.fee, lpF, pF, c.zeroForOne), c.zeroForOne, 0); // MUTANT",
      teste="test_AOneTickRangeCannotTakeTheRouteFromAnHonestPool"),
 dict(nome="V4 walk: a crossing leaves the liquidity unchanged",
      f="src/BlazePhoenixCore.sol",
      old="                liq = uint128(l);",
      new="                l; // MUTANT",
      teste="test_AOneTickRangeWithNothingBeyondPaysWhatItHolds"),
 dict(nome="V4 walk: a downward crossing applies liquidityNet with its upward sign",
      f="src/BlazePhoenixCore.sol",
      old="                if (zeroForOne) net = -net;",
      new="                // MUTANT: the downward crossing keeps the upward sign",
      teste="test_LiquidityBeyondTheCurrentRangeIsCountedWhereItIs"),
 dict(nome="V4 walk: the downward search skips the price's own compressed tick",
      f="src/BlazePhoenixCore.sol",
      old="            uint256 masked = word & (type(uint256).max >> (255 - bitPos));",
      new="            uint256 masked = word & (type(uint256).max >> (256 - bitPos)); // MUTANT",
      teste="test_AtTheLowerEdge_OnlyWhatLiesBelowIsPromised"),
 dict(nome="V4 walk: the upward search starts at the price's own compressed tick",
      f="src/BlazePhoenixCore.sol",
      old="        if (!lte) c++;",
      new="        // MUTANT: the upward search is not moved past the current tick",
      teste="test_BoundaryTick_Up_PromisesTheWholeRange"),
 dict(nome="V4 walk: positive ticks are not inverted (the table reads 1/price above tick 0)",
      f="src/BlazePhoenixCore.sol",
      old="        if (tick > 0) r = type(uint256).max / r;",
      new="        // MUTANT: no inversion for positive ticks",
      teste="test_TickMath_TheCanonicalEndpoints"),
 dict(nome="V4 walk: a step that reaches its target is charged no fee",
      f="src/BlazePhoenixCore.sol",
      old="            used = toTarget + mulDivUp(toTarget, fee, 1_000_000 - fee);",
      new="            used = toTarget; // MUTANT",
      teste="test_LiquidityBeyondTheCurrentRangeIsCountedWhereItIs"),
 dict(nome="V4 walk: one step only - the single-range model again",
      f="src/BlazePhoenixCore.sol",
