...
M = [
 ...
 dict(nome="fee: a ancora na entrada (a fuga pelos residuais)",
      f="src/BlazePhoenixRouter.sol",
      old="            if (feeIn > 0) { _payFee(tokenIn, feeIn); unchecked { amountIn -= feeIn; } }",
      new="            feeIn; // MUTANTE",
      teste="test_RotaDeFugaPagaAFeeCompleta"),
 dict(nome="fee: sobre o comprometido, nao sobre o entregue",
      f="src/BlazePhoenixRouter.sol",
      old="            if (comprometido > amountIn) comprometido = amountIn;",
      new="            comprometido = amountIn; // MUTANTE",
 ...
 dict(nome="hub: a mascara de prova de par perde um kind vivo",
      ...
      old="        | (uint256(1) << KIND_SOLIDLY) | (uint256(1) << KIND_ALGEBRA);",
      new="        | (uint256(1) << KIND_SOLIDLY); // MUTANTE",
