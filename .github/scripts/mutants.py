...
M = [
 ...
 # ── revisao 2026-09-02: T19 (pin do poolDeployer), o refutador V2/Solidly, o
 # tier no gate de frescura, o braco de remocao do allowHook, e as portas A1 ──
 # Tres revisores so de leitura sobre main 19b2f08. Cada entrada foi julgada na
 # caixa Cloudflare ANTES de entrar aqui: mutante aplicado -> o teste nomeado
 # fica vermelho, com o vizinho verde. Onde o mesmo `old` serve dois mutantes
 # (o tecto R-B), a lista aplica-os um de cada vez, por isso ambos sao validos.
 dict(nome="T19: o pin do poolDeployer na admissao cai (a origem atestada nunca e escrita)",
      f="src/BlazePhoenixHub.sol",
      old="            $.factoryDeployer[factory] = BPC.resolvePoolDeployer(factory);",
      new="            factory; // MUTANTE",
      teste="test_T19_AnswerSwap_DerivesFromAttestedDeployer"),
 dict(nome="T19: a origem da derivacao Algebra volta a ser a resposta VIVA",
      f="src/BlazePhoenixHub.sol",
 ...
 dict(nome="T19: o predicado perde o fee == 0 (um row V3 de fee 3000 deriva com o salt Algebra)",
      f="src/BlazePhoenixHub.sol",
 ...
 dict(nome="A1 porta 3 (nativa): a recusa de minOut zero cai",
      f="src/BlazePhoenixRouter.sol",
 ...
 dict(nome="A1 porta 4 (swapBestExactIn): a recusa de minOut zero cai, antes do solve",
      f="src/BlazePhoenixRouter.sol",
 ...
 dict(nome="A1 porta 5 (self-call): a porta deixa de exigir msg.sender == this",
      f="src/BlazePhoenixRouter.sol",
      old="        if (msg.sender != address(this)) revert RouterE(1);",
      new="        // MUTANTE",
      teste="test_A1_Door5_SelfCall_StrangerIsRefused"),
 dict(nome="pin de codehash das factories (modos 0-3): cai",
      f="src/BlazePhoenixHub.sol",
 ...
 dict(nome="tecto R-B multi-hop: o braco da pior leg cai (so a media gateia)",
      f="src/BlazePhoenixSolver.sol",
 ...
 dict(nome="tecto R-B multi-hop: o braco agregado cai (so a pior leg gateia)",
      f="src/BlazePhoenixSolver.sol",
      old="        if (maxLegImpactBps >= MAX_ROUTE_IMPACT_BPS || totalImpactBps >= MAX_ROUTE_IMPACT_BPS) return route;",
 ...
 dict(nome="balanceOf: um retorno vazio passa a ser recusa em vez de zero (o token por ligar)",
      f="src/BlazePhoenixCore.sol",
 ...
 dict(nome="NM-002: o piso deixa de cair para o atestado quando a quote do ultimo hop e zero",
      f="src/BlazePhoenixRouter.sol",
 ...
 dict(nome="impacto: a leg concentrada sem quote volta a contar BPS (o piso colapsa no clamp)",
      f="src/BlazePhoenixRouter.sol",
 ...
 dict(nome="swapBestExactIn: a ponta de saida do plano deixa de ser verificada",
      f="src/BlazePhoenixRouter.sol",
