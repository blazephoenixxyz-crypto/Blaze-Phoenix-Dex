...
| `expectedOut` of a single-tick leg (the per-leg attestation) | "What can this leg be promised to deliver?" - and, since the walk, the same answer to "which venue delivers more?" | produced by `Core.universalQuote`'s V4 arm through `Core.v4WalkOut`, the pool's own swap walked over its initialized ticks; the Router's in-frame quote (`Core.v4LegOut`) is the same walk, and consumes it as the per-leg floor and in Layer 1. Until 2026-09-24 the ranking figure was the edge-blind single-tick form and a separate promise layer (`Solver._promiseLegs`, #74) clamped the attestation at an edge inferred from the spacing: a pool with one tick of range out-ranked an honest one and filled 5% of the order (dex-19), and a deep pool with a narrow current range was promised a tenth of what it paid. The layer became an identity with the walk and is gone | `PINNED` | `test/V4PromiseIsWhatThePoolPays.t.sol`: `test_TheLegAttestsWhatThePoolPays_AndRankingIsThatFigure` (the attested expectedOut is what the specification's swap pays, and the route's figure is that same number) and `test_Parity_TheLegAttestsWhatTheRouterQuotesInFrame` (the Router's own ExecutionProof) |
| `v4WalkOut` (what a V4 pool pays for an input) | "What does this V4 pool pay for this amount?" | `Core.v4WalkOut` (definition): slot0, liquidity, the tick bitmap and the ticks read through extsload, the PoolManager's own step arithmetic (input rounded up, output down), at most 16 stretches - an order still unspent after them is quoted for what the walk reached. Called by `universalQuote` (ranking, the Solver's plans, the Quoter's plan preview) and `v4LegOut` (the Router's in-frame quote) | `PINNED` (2026-09-24) | `test/V4TickWalk.t.sol`: the walk against a PoolManager written from the specification with its own tick arithmetic (one range, a crossing, fuzzed books); end to end, a one-tick range cannot take the route from an honest pool, and a deep pool with a narrow current range is promised what it pays |
| `sqrtPriceAtTick` (the price at a tick) | "Where does a tick's price lie?" | `Core.sqrtPriceAtTick` alone - the TickMath table, used by the walk for every stretch's end; it replaced an edge approximated from the spacing (`sqrtBoundary`, removed) | `PINNED` (2026-09-24) | `test/V4TickWalk.t.sol`: the canonical endpoints exactly (tick 0, MIN_TICK, MAX_TICK) and every tick against an independent fixed-point computation of sqrt(1.0001)^tick |
| `_askWord` (what one pool's answer may cost a quote) | "How much gas, memory and trust may one answer take from the planner?" | `Core._askWord` - a one-word ask with `GAS_CAP` of gas and one word copied, `ok` only for a full word; the fixed-selector getters keep their inline form of the same bound (`GAS_CAP`, a fixed-size copy). `solidlyGetAmountOut` (the pair's own `getAmountOut`) and `readDynamicFee` (`factory()`, then its `getFee`) read through it; a `factory()` word that is not an address makes the pool unquotable. Until 2026-09-24 these two read with the call's whole gas and the whole returndata | `PINNED` (2026-09-24) | `test/APoolsAnswerIsBoundedByTheReader.t.sol`: a pair admitted honest whose `getAmountOut`, `factory()` or `getFee` then burns all its gas or answers a megabyte leaves the pair planned within a 50M-gas call, beside a control that plans the honest pair within the same budget; `test/FactoryAnswerIsNotTrusted.t.sol`: every `factory()` answer resolves to the fee it means. Two registered mutants (the gas cap, the dirty-word refusal) |
