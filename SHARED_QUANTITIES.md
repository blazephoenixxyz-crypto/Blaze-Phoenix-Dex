...
| `ironFloorBps` **impact input** | "What is this route's price impact?" | `Solver` ×2, `Router` ×2 — same function, **same aggregation** since PR #25 (share-weighted, rounded up) | `SINGLE` (fixed 2026-09-02) | `test/FloorParitySolverRouter.t.sol` — `singleOutFloor == floorUsed` on the same route; mutants 88-89 in `mutants.py` (was finding **FLOOR-01**) |
