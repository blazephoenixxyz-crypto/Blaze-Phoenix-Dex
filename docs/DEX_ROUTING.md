...
system and `../audit/SCOPE.md` for the review scope.

> Unaudited. Figures are fork-measured (see `../TESTING.md`).
...
Routing is exercised across single-leg, multi-hop (bridge), and intra-hop-split
shapes by stateful invariant fuzzing (pass-through: zero residual balance;
per-token conservation), and by ~85 real-liquidity fork cases on four chains. The
one known limitation is the single-tick search approximation of §5; the on-chain
floor is its backstop. See `../audit/SCOPE.md`.
