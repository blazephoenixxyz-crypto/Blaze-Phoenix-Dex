...
| | |
|---|---|
| Version | 2.2, errata applied 2026-09-07 (see the Errata section) — supersedes the 3.0 numbering of 2026-09-01; this is the canonical version line |
| Date | 2026-09-06 |
| Author | Mitra |
...
A static-fee V4 pool carries its LP fee in its key, where it is immutable. The manager may additionally charge a protocol fee — at most 0.1 % per direction — that the Core's static arm does not add, so a quote on a static key can overstate the delivered output by up to that protocol fee once the fee controller enables it (it is zero on every chain served at the time of writing); the shortfall lands inside the floor, and the fee base, being measured, ignores it. A dynamic-fee pool carries the sentinel `0x800000` instead, and its real fee lives in `slot0`. Quoting the sentinel at face value would price the trade as free while execution paid the live rate. The gate is three lines in the Core (`effV4Fee`):
...
**The aggregate floor per hop.** The per-leg floor is local while composition is global: an attacker holding one leg of $L$ could extract about $20\,\% \cdot (L-1)/L$ of a hop without failing any single floor. So each hop may lose at most what one *average* attested leg could legitimately lose: $\sum \mathrm{got} + \lfloor (\mathrm{BPS} - \mathrm{LEG\_FLOOR\_BPS}) \cdot \lfloor \sum \mathrm{attested} / n \rfloor / \mathrm{BPS} \rfloor \ \ge\ \sum \mathrm{attested}$ — the slack is 20 % of the average attested leg, else `RouterE(5)`. The mean and not the maximum, because under a maximum the attacker inflates their own leg to inflate the shared budget. For one leg the rule collapses exactly onto the per-leg floor — zero new rigidity, by construction.
...
| Denial of service by unbounded iteration (DASP-5) | every route dimension bounded: `MAX_HOPS = 3`, `MAX_LEGS_PER_HOP = 5`, `MAX_LEGS = 11`, `MAX_CANDIDATES = 8`, `MAX_SLOTS = 16`, `MAX_FACTORIES = 16` | Router `_execute` → `RouterE(3)`; Solver; Hub → `HubE(4)` (factories), `_canInsert` (slots) | blocked | `test/RouteHopCeiling.t.sol` |
...
|---|---|---|
| declared tests | 1,478 `test*` / `invariant*` / `check*` functions across 217 `.t.sol` files | `grep` over `test/` |
...
| stateful invariants | 43 `invariant_*` functions in 14 campaigns | `grep -rc 'function invariant_' test` |
...
![FIG-13 — Mutants aimed at the invariants: fifteen source mutations against forty-three stateful invariants, the invariants that went red per mutant, the three closed on 2026-09-05 by giving a campaign the action it lacked, and the one sentinel that survives by design.](figs/fig-13.svg)
...
| E24 | Aggregate hop floor | $\sum \mathrm{got} + \lfloor (\mathrm{BPS} - 8{,}000)\cdot \lfloor \sum \mathrm{attested}/n \rfloor / \mathrm{BPS} \rfloor \ge \sum \mathrm{attested}$ | Router `_execute` | §11.3 |
...
| INV-8 | A pair's registry holds at most sixteen pools, and eviction keeps the fittest by full Ψ with the margin of E13. | Hub `_canInsert`; `invariant_neverExceedsMaxSlots` (the cap); `test_RecordSwap_EvictsWeakestWhenFullAndNewcomerClearsMargin` (the eviction) |
| INV-9 | No address without runtime bytecode enters a route. | `hasCode` on every derivation (§7) |
| INV-10 | The universal callback pays only the pool committed for the current leg, only while a leg is in flight, and never more than the leg's recorded budget. | Transient leg context; `RouterE(6)` for the pool, `RouterE(8)` for the budget (§11.4) |
...
| INV-15 | The fee base is the measured pull, capped by the route's committed input — never a declared output; `route.totalOut` is read by nothing. | `_chargeHopFee`; `test_FeeBase_IgnoresLiedAboutTotalOut` |
...
| INV-20 | A settlement pays the protocol fee exactly once on an anchored route and once per hop on an exhausted one; a settlement that paid nothing does not exist. | Fee ledger, `RouterE(15)`/`RouterE(16)`; `invariant_SettledSwapEmitsExactlyOneFee` (anchored); `test/FeeSeals.t.sol`, `test/ExhaustionRegimePreviewParity.t.sol` (exhaustion) |
| INV-21 | Hops chain — each hop's input is the previous hop's output — and no hookless leg executes after a hooked one, in this or any later hop (`sawHooked` is route-wide). | Router `_execute` → `RouterE(3)`; `sawHooked` |
| INV-22 | The four deployed contracts — Hub, Solver, Router, Quoter — ship under the project's 24,000-byte gate on the release profile, the Core library is measured separately (6,477 bytes of 24,576), and the suite runs on that profile. | `DeployedSizeGate`; `profile_parity.py` |
...
# the factory census (§17.3): harness in the private deployment tree, not reproducible from this checkout
