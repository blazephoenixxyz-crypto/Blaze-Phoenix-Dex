...
- **The mutation guard** — 268 curated mutants, each paired with the one test that must die (fourteen of them name a stateful `invariant_*` — see `docs/assurance/invariant-mutants.json` for the matrix of 15 mutants × 40 invariants and the two guards it found unwatched);
...
- **The Router campaign, two hops and two legs** — `BlazePhoenixRouter.invariant.t.sol` walks the chain through two adjacent pairs, splits hop 0 across two pools, and asserts the fee shape the rule prescribes from the bridge list and the pools (`invariant_TwoHopFeeShapeFollowsTheRule`); its detection column in `docs/assurance/fee-seal-detection.json` was blind to two fee mutants before the widening and sees them 20/20 after.
