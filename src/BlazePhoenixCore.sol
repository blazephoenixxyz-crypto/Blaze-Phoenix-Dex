...
library BlazePhoenixCore {
    ...
    // loosens toward the 75% hard cap as impact, leg count and volatility rise.
    // Base and hard cap are separate so the floor is impact-adaptive while
    // never permitting a result worse than 75% of the quote.
    uint16  internal constant FLOOR_HARD_MAX_LOSS_BPS = 2_500; // 25% absolute hard cap (floor ≥ 75%)
    ...
    function balanceOf(address token, address who) internal view returns (uint256 b) {
        assembly ("memory-safe") {
            ...
            if staticcall(gas(), token, m, 36, m, 32) { b := mload(m) }
        }
    }
    ...
    /// @notice Vitality score from slot (EMA decay over block window).
    function vitality(uint256 slot, uint32 currentBlock) internal pure returns (uint256 v) {
        ...
        // base: swap count, decayed by age window of 65 536 blocks (~9 days @ 12s).
        if (age >= 16) {
            uint256 shift = age >> 4;
    ...
    function psi(
        uint256 slot, uint32 currentBlock,
        bool isBridge, bool isConcentrated
    ) internal pure returns (uint256 f) {
        uint256 v = vitality(slot, currentBlock);
        if (v == 0) return 0;
    ...
    //  leg count, and is clamped so it never drops below the 75% hard cap
    //  (BPS − 2 500 = 7 500) regardless of inputs.
